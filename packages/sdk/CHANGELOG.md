...
- 1fe3c66: Add the `blocked` message lifecycle status.

  `blocked` is a terminal status set when a message is stopped by automated
  content-safety checks. This brings the SDK, CLI, and MCP surface back in
  lockstep with the app's `messageStatusEnum`.
